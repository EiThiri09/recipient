...
function RecipientsDisplay({ recipients }: RecipientsDisplayProps) {
    ...
    useEffect(() => {
        const handleResize = () => {
            ...
            if (recipientSpan) {
                ...
                const width = (Math.round(recipientSpan.getBoundingClientRect().width) - wordWidth); //wordWidth is for , ...
                //console.log("check container width", Math.round(recipientSpan.getBoundingClientRect().width), tempSpan.offsetWidth, width)
    ...
    const displayRecipient = (recipients: string[]) => {
        ...
        if (recipients.length === 1) {
            ...
            recipients.forEach((item: string, index: number) => {
                const tempSpan = document.createElement('span');
                tempSpan.textContent = wordsThatFitArray !== "" ? wordsThatFitArray + ", " + item : item;
                tempSpan.style.visibility = 'hidden';
                document.body.appendChild(tempSpan);
                let wordWidth = tempSpan.offsetWidth;
    ...
    return (
        <Container id="recipientId"><StyledRecipient ref={recipientsRef}>{displayRecipient(recipients)}</StyledRecipient> {remainingCount > 0 && <StyledCount>+{remainingCount}</StyledCount>}</Container>
    );
};
